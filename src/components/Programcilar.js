...
export default function Programcilar() {
  ...
  const [programcilar, setprogramcilar] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    const secilen = enIyiler.find((item) => {
      return item.id == programcilar;
    });
    return secilen.isim;
  };
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: programcilar == null ? "royalblue" : "gold", // 🤔 kutlarken renk gold'a dönecek
  };
